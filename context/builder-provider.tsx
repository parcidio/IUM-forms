...
type BulderContextType = {
  ...
  // Added: replace entire block layout instance (used by properties editors)
  updateBlockLayoutInstance: (
    id: string,
    updatedInstance: FormBlockInstance
  ) => void;
};
...
export default function BuilderContextProvider({
  ...
  // New: replace the whole layout instance (keeps list order, updates selected if needed)
  const updateBlockLayoutInstance = (
    id: string,
    updatedInstance: FormBlockInstance
  ) => {
    setBlockLayouts((prev) =>
      prev.map((block) => (block.id === id ? { ...updatedInstance } : block))
    );
    if (selectedBlockLayout?.id === id) {
      setSeletedBlockLayout(updatedInstance);
    }
  };
